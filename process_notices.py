...
def make_prob_txt(params):
  ...
  P_HasRemnant = float(params['HasRemnant'])
  P_HasNS = float(params['HasNS'])
  ...
  prob_txt += "\n"
  prob_txt += "with P(HasRemnant) = %.2f and P(HasNS) = %.2f\n"%(P_HasRemnant, P_HasNS)
...
def respond_burst(root, params):
  alert_type = params['AlertType']
  
  sms_txt = "%s notice\n"%(alert_type)
  sms_txt += "This is event is an *Unmodelled Burst*\n\n"
  sms_txt += "More info here: %s"%(params['EventPage'])
  ...
  event_txt = "*%s Notice:* %s detected with %s and FAR %.2G /yr\n*Time of Signal:* %sUTC\n\n"%(alert_type, params['GraceID'], params['Instruments'], FAR, event_time)
  
  event_txt += "This event is an *Unmodelled Burst*"
  ...
  elif alert_type == 'Initial' or alert_type == 'Update':
    vetted_txt = "This notice *HAS* been vetted by humans"
    
  else:
    vetted_txt = 'This event has been retracted after human vetting.'
    event_txt = "*Retraction:* %s"%(params['GraceID'])
    
  message_txt = '%s\n\n%s. %s'%(event_txt, vetted_txt, eventpage_txt)

  return message_txt, sms_txt
...
def process_gcn(payload, root):
  ...
  if params['Packet_Type'] == "164":
    ...
    if group == 'Burst':
      message_txt, sms_txt = respond_burst(root, params)
  ...
  if observability_plot:
    if params['Packet_Type'] != "164" and group == 'CBC':
      try:
...
def run_sms_test():
  ...
  test_files = ['example_burst_initial.xml', 'example_initial.xml']
...
def run_output_test():
  ...
  test_files = ['example_preliminary.xml', 'example_initial.xml', 'example_update.xml', 'example_retraction.xml', 'example_burst_preliminary.xml', 'example_burst_initial.xml', 'example_bad_url.xml']
  ...
  url = 'example_burst_preliminary.xml'
  payload = astropy.utils.data.get_file_contents(url)
  root = lxml.etree.fromstring(payload)
  process_gcn(payload, root)
